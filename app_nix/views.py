...
from .models import Leads, Anunciante, Tagmeta, TagGoogle, TagGoogleBody, AnuncioVeiculo, Cores
...
def index(request):
    listagem = AnuncioVeiculo.objects.all()
    busca = request.GET.get('barra_pesquisa')
    if busca:
        listagem = AnuncioVeiculo.objects.filter(
...
def dashboard(request, user):
    ...
    perfil_user = Anunciante.objects.last()
...
def estoque_veiculos(request, user):
    if request.user.username == user:
        print('PROPRIETARIO DE ESTOQUE')
        veiculo = AnuncioVeiculo.objects.filter(anunciado_por=user)
        busca = request.GET.get('barra-pesquisa')
        if busca:
            veiculo = AnuncioVeiculo.objects.filter(
        ...
        print(mensagem)
        return render(request, 'site/dashboard-gerenciador-estoque.html', {'veiculo': mensagem})


def deletar_lp(request, pk):
    if request.method == "POST":
        lp = AnuncioVeiculo.objects.filter(pk=pk)
        print(lp)
        lp.delete()
...
def configuracao(request, user):
    ...
    if user == logado:
        perfil = Anunciante.objects.filter(user_vinculado=user)
        print(perfil)
...
def add_img_perfil(request):
    if request.method == 'POST':
        ...
        Anunciante.objects.create(img_perfil=img_perfil,
...
def editar_img_perfil(request):
    if request.method == 'POST':
        ...
        Anunciante.objects.get(user_vinculado=user_vinculado).delete()
        Anunciante.objects.create(img_perfil=img_perfil,
                                  user_vinculado=user_vinculado)
        return redirect(reverse('configuracao', args=[request.user]))
...
def landingpage(request, pk, slug):
    if request.method == 'POST':
        ...
        veiculo = get_object_or_404(AnuncioVeiculo, pk=pk, slug=slug)
...
@login_required
def cadastrar_veiculos(request, user):
    if request.method == 'POST':
        ...
        AnuncioVeiculo.objects.create(nome_modelo=modelo,
                                      nome_marca=marca,
        ...
        lp = AnuncioVeiculo.objects.last()
        return render(request, 'site/upload-img.html', {'lp': lp})
    else:
        cores = Cores.objects.all()
        return render(request, 'site/cadastrar-veiculos.html', {'cores': cores})
...
def editar_lp(request, pk):
    if request.method == 'POST':
        ...
        # instanciando objeto
        lp_sendo_editada = get_object_or_404(AnuncioVeiculo, pk=pk)
        ...
        lp_relacionada = get_object_or_404(AnuncioVeiculo, pk=pk)
        print(lp_relacionada)

        return render(request, 'site/upload-img-edicao.html', {'lp': lp_relacionada})
    else:
        lp_editar = get_object_or_404(AnuncioVeiculo, pk=pk)
        print(lp_editar)
...
def cadastro_lp(request, slug):
    empreendimento_relacionado = get_object_or_404(AnuncioVeiculo, slug=slug)
...
def cadastrar_lp(request):
    if request.method == 'POST':
        ...
        AnuncioVeiculo.objects.create(nome_empreendimento=nome_empreendimento,
        ...
        return render(request, 'site/cadastrar-veiculos.html')


def upload_img(request, user):
    if request.method == 'POST':
        veiculo_vinculado = request.POST.get('veiculo_vinculado')
        # Obtém a instância do objeto AnuncioVeiculo ou retorna 404 se não existir
        incluindo_img_anuncio_veiculo = get_object_or_404(AnuncioVeiculo, pk=veiculo_vinculado)

        # Obtém as imagens enviadas através do campo 'file' no formulário
        imagens = request.FILES.getlist('file')

        # Lógica para salvar as imagens na instância do objeto AnuncioVeiculo
        for idx, img in enumerate(imagens, start=1):
            if idx <= 10:  # Certifique-se de que não estamos excedendo o número máximo de imagens
                ...
                setattr(incluindo_img_anuncio_veiculo, image_field, img)

        # Salva o objeto AnuncioVeiculo no banco de dados após adicionar as imagens
        incluindo_img_anuncio_veiculo.save()
